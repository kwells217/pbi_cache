...
function getAuthHeader(method, blobName, contentLength = '', contentType = '') {
  const now = new Date().toUTCString();
  const storageKey = Buffer.from(STORAGE_KEY, 'base64');
  const canonicalHeaders = `x-ms-date:${now}\nx-ms-version:2020-10-02`;
  const canonicalResource = `/${STORAGE_ACCOUNT}/${CACHE_CONTAINER}/${blobName}`;
  const stringToSign = [
    method.toUpperCase(),
    '', '', contentType, '', '', '', '', '', '', '', '',
    canonicalHeaders,
...
async function writeBlobCache(data) {
  try {
    ...
    await fetch(containerUrl, { method: 'PUT', headers: { ...createHeaders, 'Content-Length': '0' } });
    const content = JSON.stringify({ timestamp: Date.now(), data });
    const buf = Buffer.from(content, 'utf8');
    const writeHeaders = {
      ...getAuthHeader('PUT', CACHE_BLOB, buf.length, 'application/json'),
      'x-ms-blob-type': 'BlockBlob',
      'Content-Type': 'application/json',
      'Content-Length': buf.length
    };
    await fetch(blobUrl(CACHE_BLOB), { method: 'PUT', headers: writeHeaders, body: content });
