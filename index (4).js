...
function blobUrl(blob) {
  return `https://${STORAGE_ACCOUNT}.blob.core.windows.net/${CACHE_CONTAINER}/${blob}`;
}
...
function getAuthHeader(method, blobName, contentType = '') {
  const now = new Date().toUTCString();
  const storageKey = Buffer.from(STORAGE_KEY, 'base64');
  const canonicalHeaders = `x-ms-date:${now}\nx-ms-version:2020-10-02`;
  const canonicalResource = blobName
    ? `/${STORAGE_ACCOUNT}/${CACHE_CONTAINER}/${blobName}`
    : `/${STORAGE_ACCOUNT}/${CACHE_CONTAINER}\nrestype:container`;
  const stringToSign = [
    method.toUpperCase(),
    '', '', contentType, '', '', '', '', '', '', '', '',
    canonicalHeaders,
...
async function readBlobCache() {
  try {
    const headers = getAuthHeader('GET', CACHE_BLOB);
    const res = await fetch(blobUrl(CACHE_BLOB), { headers });
...
async function writeBlobCache(data) {
  try {
    ...
    const containerUrl = `https://${STORAGE_ACCOUNT}.blob.core.windows.net/${CACHE_CONTAINER}?restype=container`;
    const createHeaders = getAuthHeader('PUT', '');
    const containerRes = await fetch(containerUrl, {
      method: 'PUT',
      headers: { ...createHeaders, 'Content-Length': '0' }
    });
    // 201 = created, 409 = already exists — both are fine
    if (containerRes.status !== 201 && containerRes.status !== 409) {
      console.error('Container create failed:', containerRes.status, await containerRes.text());
    }
    ...
    const writeHeaders = {
      ...getAuthHeader('PUT', CACHE_BLOB, 'application/json'),
      'x-ms-blob-type': 'BlockBlob',
      'Content-Type': 'application/json',
      'Content-Length': String(contentLength)
    };
    const blobRes = await fetch(blobUrl(CACHE_BLOB), {
      method: 'PUT',
      headers: writeHeaders,
      body: content
    });
    if (!blobRes.ok) {
      console.error('Blob write failed:', blobRes.status, await blobRes.text());
    }
